...
pub fn localized_managed_skills(locale: &str) -> Vec<ActivatedSkill> {
    match locale {
        "ja" => ja_managed_skills(),
        _ => en_managed_skills(),
    }
}

// Each managed skill is a flat literal definition; splitting further would only
// add indirection without separating concerns.
#[allow(clippy::too_many_lines)]
fn ja_managed_skills() -> Vec<ActivatedSkill> {
    vec![
        managed_skill(
            "research-brief",
            "1.0.0",
...
// add indirection without separating concerns.
#[allow(clippy::too_many_lines)]
fn en_managed_skills() -> Vec<ActivatedSkill> {
    vec![
        managed_skill(
            "research-brief",
            "1.0.0",
